            ...
            <div class="col-lg-7 offset-lg-1">
                <div class="course-nav d-flex justify-content-between">
                    ...
                    <div class="course_page_next_box text-end">
                        <a href="" class="btn-default btn-secondary btn-150" onclick="event.preventDefault();document.getElementById('quiz_form').checkValidity() ? document.getElementById('quiz_form').submit() : alert('Please answer all the questions!') ;">
                            SUBMIT
                ...
                <div class="course-description">
                    <p>{!!$course_sequence[$desired_sequence]->detail!!}</p>
                </div>
                <form id="quiz_form" action="{{route('course.teacher.course_quiz_submited')}}" method="POST">

                    <input type="hidden" name="courseID" value={{$course->id}} id="">

                    <input type="hidden" name="quizID" value={{$course_sequence[$desired_sequence]->id}} id="">

                    <input type="hidden" name="sequence_which_just_been_watched" value={{$desired_sequence}} id="">
    
                    @csrf
                    <!-- Accordions Bordered -->
                    <div class="quiz-accordion">
                        @foreach($course_sequence[$desired_sequence]->questions as $question)
                            ...
                            <div class="card-body">
                                <p class="card-text">
                                    <div class="form-check form-radio-secondary mb-3">
                                        <input class="form-check-input" type="radio" name="{{$question->id}}" value="1" id="formradioRight{{$question->id}}_1" required>
                                        <label class="form-check-label" for="formradioRight{{$question->id}}_1">
                                            {{$question->option_1}}
                                        </label>
                                    </div>
                                    <div class="form-check form-radio-secondary mb-3">
                                        <input class="form-check-input" type="radio" name="{{$question->id}}" value="2"  id="formradioRight{{$question->id}}_2" required>
                                        <label class="form-check-label" for="formradioRight{{$question->id}}_2">
                                            {{$question->option_2}}
                                        </label>
                                    </div>
                                    <div class="form-check form-radio-secondary mb-3">
                                        <input class="form-check-input" type="radio" name="{{$question->id}}" value="3" id="formradioRight{{$question->id}}_3" required>
                                        <label class="form-check-label" for="formradioRight{{$question->id}}_3">
                                            {{$question->option_3}}
                                        </label>
                                    </div>
                                    <div class="form-check form-radio-secondary mb-3">
                                        <input class="form-check-input" type="radio" name="{{$question->id}}" value="4" id="formradioRight{{$question->id}}_4" required>
                                        <label class="form-check-label" for="formradioRight{{$question->id}}_4">
                                            {{$question->option_4}}
                                        </label>
                                    </div>
                                </p>
